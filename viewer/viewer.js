...
var PDFView = {
  ...
  open: function pdfViewOpen(url, scale) {
    ...
    req.onsuccess = function(e) {
      ...
      var link = decodeURIComponent(url).substr(52);

      var reqRead = db.transaction("elibri").objectStore("elibri").get(link);
      reqRead.onsuccess = function(eR) {
        var result = eR.target.result;
        document.title = result.book[0] + " - " + result.book[1];
        if(result.pdf_file) {
          self.loading = true;
          self.load(result.pdf_file, scale);
          self.loading = false;
        }
        else {
          PDFJS.getPdf(
    ...
    req.onerror = function(e) {
      ...
      loadingIndicator.innerHTML = 'Error';
      var moreInfo = {
        message: 'Unexpected database error: ' + e.target.errorCode + '.'
      };
      self.error('An error occurred while loading the PDF.', moreInfo);
    }
  },
